...
window.addEventListener("DOMContentLoaded", () => {
	...
	if (flag) {
		...
		body.remove();
		html.appendChild(timer);

		const year = today.getFullYear();
		let valentineDate = new Date(year, 1, 14); // 1 یعنی فوریه
		...
		function updateTimer() {
			...
			if (timeDiff <= 0) {
				timer.textContent = "ولنتاین مبارک! ❤️";
				return;
			}
			...
			timer.innerText = "به زودی اینجا یه شهره بازی ساخته میشه 👀" 
		}
...
window.onload = function () {
	...
	function openBox() {
		if (step === 1) {
			...
			let endGame = localStorage.getItem('endGameValue')

			if (!endGame) {
				confirm('Error 404 Your gift is stolen by $&*@*!#')
				if (confirm || !confirm) {
